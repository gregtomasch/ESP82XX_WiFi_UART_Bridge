...
void WiFiSerial::serialCom()
{
  ...
  for(n=1; n < UART_NUMBER; n++)                                                                                          // General function to handle multiple active ports. Limited to UART1 - UART3 for the IU Butterfly-based board
  {
    CURRENTPORT = n;
...
void WiFiSerial::evaluateCommand()
{
  switch(cmdWiFiSerial[CURRENTPORT])
  {
   ...
   case SEND_SERIAL_DATA:                                                                                                 // CMD 20
     cmdWiFiSerial[BUTTERFLY_WIFI_UART] = 10;
     WiFiSerial::headSerialRequest(BUTTERFLY_WIFI_UART, 0, 4*(UPDATE_SIZE));
     for(uint8_t i=0; i < (UPDATE_SIZE-1); i++)
     {
       WiFiSerial::serializefloat(data_update[i]);
     }
     WiFiSerial::tailSerialRequest(BUTTERFLY_WIFI_UART);
   default:
     break;
  }
}
