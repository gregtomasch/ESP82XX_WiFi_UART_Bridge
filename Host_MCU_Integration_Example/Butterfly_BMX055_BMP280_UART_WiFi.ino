...
// Pin definitions
// The BMX055 has three sensors, and two interrupt pins per device! Just use the accel interrupt INT2 on pin D7
int           intPin   =   7; 
bool          newData = false;
int           myLed     = A0;                                                                              // Blue ledD on the IU Butterfly
...
void setup()
{
  ...
  // ************* Open the UART bridge serial port *************
  WiFiSerial::SerialOpen(BUTTERFLY_WIFI_UART, 115200);
  ...
  digitalWrite(myLed, LOW);                                                                                // Turn on led, active LOW
  ...
  Serial.print("BMX055 ACC"); Serial.print(" I AM 0x"); Serial.print(c, HEX);
  Serial.print(" I should be 0x"); Serial.println(0xFA, HEX);
  ...
  Serial.print("BMX055 GYRO"); Serial.print(" I AM 0x"); Serial.print(d, HEX);
  Serial.print(" I should be 0x"); Serial.println(0x0F, HEX);
  ...
  Serial.print("BMX055 MAG"); Serial.print(" I AM 0x"); Serial.print(e, HEX);
  Serial.print(" I should be 0x"); Serial.println(0x32, HEX);
  ...
  {  
    ...
    Serial.println("accel biases (mg)"); Serial.println(1000.*accelBias[0]);
    Serial.println(1000.*accelBias[1]); Serial.println(1000.*accelBias[2]);
    Serial.println("gyro biases (dps)"); Serial.println(gyroBias[0]);
    Serial.println(gyroBias[1]); Serial.println(gyroBias[2]);
    magcalBMX055(magBias);
    Serial.println("mag biases (mG)"); Serial.println(magBias[0]);
    Serial.println(magBias[1]); Serial.println(magBias[2]);
...
{  
  ...
  if (delt_t > 500)                                                                                        // Update LCD once per half-second independent of read rate
  {
    ...
    temperature = ((float) tempCount) / 2.0f + 23.0f;                                                      // Gyro chip temperature in degrees Centigrade
    Serial.print("Gyro temperature = "); Serial.print( temperature, 2); Serial.println(" C");
    TimeStamp = ((float)(micros() - Start_time))/1000000.0f;                                               // Timestamp

    // ************* Load update data into the update array and push over the UART *************
    data_update[0] = TimeStamp; data_update[1] = ax*1000.0f; data_update[2] = ay*1000.0f; data_update[3] = az*1000.0f; data_update[4] = gx;
    data_update[5] = gy; data_update[6] = gz; data_update[7] = mx; data_update[8] = my; data_update[9] = mz;
    data_update[10] = q[0]; data_update[11] = q[1]; data_update[12] = q[2]; data_update[13] = q[3]; data_update[14] = temperature;
    data_update[15] = pressure; data_update[16] = altitude; data_update[17] = yaw; data_update[18] = pitch; data_update[19] = roll;
    WiFiSerial::Push_Data_Update();
...
void getGres()
{
  switch (Gscale)
  {
    ...
    case GFS_125DPS:
      gRes = 124.87f/32768.0f;                                                                             // Per data sheet, not exactly 125!?
      break;
    case GFS_250DPS:
      gRes = 249.75f/32768.0f;
      break;
    case GFS_500DPS:
      gRes = 499.5f/32768.0f;
      break;
    case GFS_1000DPS:
      gRes = 999.0f/32768.0f;
      break;
    case GFS_2000DPS:
      gRes = 1998.0f/32768.0f;
      break;
  }
}
...
void readMagData(int16_t * magData)
{
  ...
  {                                                                                                        // Check if data ready status bit is set
    ...
    magData[0] = ((int16_t)((((int32_t)mdata_x) * ((((((((int32_t)dig_xy2) * ((((int32_t)temp) * ((int32_t)temp)) >> 7)) +
	  (((int32_t)temp) * ((int32_t)(((int16_t)dig_xy1) << 7)))) >> 9) + ((int32_t)0x100000)) * ((int32_t)(((int16_t)dig_x2) +
	  ((int16_t)0xA0)))) >> 12)) >> 13)) + (((int16_t)dig_x1) << 3);
    temp = ((int16_t)(((uint16_t)((((int32_t)dig_xyz1) << 14)/(data_r != 0 ? data_r : dig_xyz1))) - ((uint16_t)0x4000)));
    magData[1] = ((int16_t)((((int32_t)mdata_y) * ((((((((int32_t)dig_xy2) * ((((int32_t)temp) * ((int32_t)temp)) >> 7)) + 
    (((int32_t)temp) * ((int32_t)(((int16_t)dig_xy1) << 7)))) >> 9) + ((int32_t)0x100000)) * ((int32_t)(((int16_t)dig_y2) +
    ((int16_t)0xA0)))) >> 12)) >> 13)) + (((int16_t)dig_y1) << 3);
...
void magcalBMX055(float * dest1) 
{
  ...
  for(ii = 0; ii < sample_count; ii++)
  {
   ...
   dest1[0] = (float) mag_bias[0]*mRes;                                                                     // Gave mag biases in G for main program
